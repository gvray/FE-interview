...
下面以文字描述形式说明 HTTP 缓存决策流程图：

1. 客户端发起请求，先检查本地是否有缓存副本。
2. 若无缓存：直接向服务器发送请求，服务器返回资源及缓存指令（`Cache-Control`、`Expires`、`ETag`、`Last-Modified`），客户端将响应存入缓存。
3. 若有缓存：
   - 判断强缓存（`Cache-Control: max-age` 或 `Expires`）是否过期。
   - 未过期：直接使用本地缓存，返回 `200 (from disk/memory cache)`，不与服务器通信。
   - 已过期：进入协商缓存阶段，向服务器发送条件请求：
     - 若上次响应带有 `ETag`，请求头携带 `If-None-Match`。
     - 若上次响应带有 `Last-Modified`，请求头携带 `If-Modified-Since`。
4. 服务器比对资源：
   - 资源未变化：返回 `304 Not Modified`，客户端继续使用本地缓存。
   - 资源已变化：返回 `200 OK` 及新的资源与缓存指令，客户端更新缓存。
5. 控制指令示例：`Cache-Control: no-cache` 表示每次都走协商缓存；`no-store` 表示完全不缓存；`public/private` 控制中间缓存是否可缓存；`must-revalidate` 表示过期后必须重新校验。

（此处为缓存流程文字说明，若有需要可后续补充流程示意图）
