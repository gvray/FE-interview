...
ES6 的 `class extends` 本质上就是基于原型链的寄生组合式继承，使用 Babel 编译后大致等价于：
...
function _getPrototypeOf(o) {
  return Object.setPrototypeOf ? Object.getPrototypeOf(o) : o.__proto__;
}
```

可以看到编译后的核心就是我们前面讲到的寄生组合式继承：`Child.prototype = Object.create(Parent.prototype)` 同时保留 `constructor`，并把子类构造函数的 `__proto__` 指向父类构造函数，从而支持 `Child extends Parent` 的静态继承。
