...
在 JavaScript 中，函数是一等公民（First-class Citizen），这意味着函数可以像其他数据类型一样被对待：可以赋值给变量、作为参数传递给其他函数、也可以作为函数的返回值。正是因为函数是一等公民，JavaScript 才能支持回调、高阶函数、闭包、柯里化等特性，这也是函数式编程得以实现的基础。
...
1. **语法简洁**：箭头函数省略了 `function` 关键字，单条表达式可省略 `return` 和花括号，让代码更紧凑。
...
2. **词法 `this`**：箭头函数没有自己的 `this`，它会从外层作用域继承 `this`，避免了 `that = this`、`bind(this)` 这类写法，特别适合回调函数。

```js
function Timer() {
  this.count = 0;
  setInterval(() => {
    // 这里的 this 指向 Timer 实例，而不是 window
    this.count++;
  }, 1000);
}
```

3. **没有自己的 `arguments`、`super`、`new.target`**：它们都由外层作用域决定，简化了部分场景的参数获取（可用 `...rest` 替代 `arguments`）。

4. **不能作为构造函数**：箭头函数没有 `[[Construct]]` 内部方法和 `prototype` 属性，使用 `new` 调用会抛出 `TypeError`，因此适合作为工具函数或回调，而非用来定义类型实例。

5. **适合作为回调**：由于语法简洁且 `this` 绑定明确，箭头函数非常契合 `map`、`filter`、`setTimeout` 等回调场景。

**使用注意事项（不适合的场景）：**

- **对象方法**：对象内定义的方法如果依赖动态 `this`，使用箭头函数会导致 `this` 指向外层作用域而非对象本身。
- **原型方法**：同理，原型上的方法通常需要通过实例访问并绑定 `this`。
- **需要 `arguments` 对象**：应使用普通函数或 `...rest` 参数。
- **事件监听需要 `this` 指向触发元素**：如 `addEventListener` 中需要 `this` 指向绑定元素时，不宜使用箭头函数。

```js
// 反例：对象方法中使用箭头函数
const obj = {
  name: 'Gavin',
  sayHi: () => console.log(this.name), // this 指向外层，不指向 obj
};
obj.sayHi(); // undefined
```
