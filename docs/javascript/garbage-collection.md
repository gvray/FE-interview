...
为了解决引用计数带来的循环引用问题，现代浏览器在标记清除的基础上引入了优化策略：从一组"根对象"（如 `window`、`document`、栈上的局部变量等）出发，递归遍历所有可达对象，不可达的对象即被回收。这样一来，即使存在相互引用，只要它们从根对象不可达，就会被正确回收。

此外，V8 引擎还采用了分代回收（Generational Collection）策略：

- 新生代（Scavenge 算法）：存放短生命周期对象，采用 From/To 两块空间复制式回收，存活对象被复制到 To 空间。
- 老生代（Mark-Sweep / Mark-Compact）：存放长生命周期对象，先标记清除，再整理内存碎片。

常见内存泄露的场景包括：意外的全局变量、被遗忘的定时器或回调、闭包中不必要的引用、脱离 DOM 的引用以及循环引用。开发中应避免这些情况，必要时借助 Chrome DevTools 的 Memory 面板进行排查。
