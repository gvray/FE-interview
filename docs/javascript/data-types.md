...
JavaScript 在进行 `==`（宽松相等）比较时会进行类型转换，规则遵循 ECMAScript 的抽象相等比较算法：

- 两者类型相同：直接比较（注意 `NaN` 不等于任何值，包括自身；`+0` 与 `-0` 相等）。
- `null` 与 `undefined`：两者宽松相等，且与其它类型都不相等。
- `Number` 与 `String`：将字符串转换为数字再比较，如 `1 == '1'` 为 `true`。
- `Boolean` 与其它类型：先将布尔值转换为数字（`true → 1`，`false → 0`），再按其它规则比较。
- `Object` 与基本类型：先调用对象的 `valueOf`，若结果仍不是基本类型再调用 `toString`，得到基本类型后再比较。
- `Symbol` 与其它类型：除 `Object` 转 `Symbol` 之外，`Symbol` 不与任何值宽松相等。

示例：`[] == false` 过程为 `[] → '' → 0`，`false → 0`，因此结果为 `true`；而 `{} == false` 会变成 `[object Object] → NaN`，结果为 `false`。

推荐始终使用 `===`（严格相等）来避免隐式转换带来的歧义。

### JS中Map和Object的区别

- 键的类型：`Object` 的键只能是字符串或 `Symbol`；`Map` 的键可以是任意类型，包括对象、函数、基本类型。
- 顺序：`Map` 保留插入顺序，遍历时按插入顺序输出；`Object` 的字符串键虽然大多数引擎也按插入顺序，但整数键会提前并按升序排列，`Symbol` 键的顺序也不保证。
- 大小：`Map` 直接通过 `map.size` 获取键值对数量；`Object` 需要 `Object.keys(obj).length` 间接计算。
- 性能：`Map` 在频繁增删键值对的场景下性能更好；`Object` 在静态结构、JSON 序列化方面更方便。
- 原型：`Object` 有原型链，可能存在 `toString`、`hasOwnProperty` 等原型属性的冲突；`Map` 不会受原型影响，键空间更干净。
- 序列化：`Object` 可以直接用 `JSON.stringify` 序列化；`Map` 默认不支持 JSON，需要自行转换。
- 迭代：`Map` 实现了 `Symbol.iterator`，可直接用 `for...of` 或 `forEach` 遍历；`Object` 需借助 `Object.keys/values/entries`。

一般情况下，键值对结构简单、需要 JSON 序列化时用 `Object`；键类型多样、频繁增删、需要有序遍历时用 `Map`。
